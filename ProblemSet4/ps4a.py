...
def updateHand(hand, word):
    ...
    for letter in word:
        if letter in hand:
           handUp[letter] = handUp.get(letter) - 1
    return handUp
...
def isValidWord(word, hand, wordList):
    ...
    result = 0
    for letter in word:
        if letter in hand.keys() and word.count(letter) <= hand.get(letter) and word in wordList:
            result +=1
        else:
            return False
    if result == len(word):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(uHand) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            if not isValidWord(word, uHand, wordList):
                # Reject invalid word (print a message followed by a blank line)
                print('Invalid word, please try again.')
                print()
            # Otherwise (the word is valid):
            if isValidWord(word, uHand, wordList):
